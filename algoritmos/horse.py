...
# [ [ [nodo1, nodo2, nodo3], nodo2, nodo3, nodo4] ]
    ...
    def movimientos(self) -> list:
        ...
        lista_movimientos = []
        nuevo_mundo_aux = None

        for i in range(8):
            if self.verifica_movimiento(self.direcciones[DIRECCIONES[i]] ):
                x = self.direcciones[DIRECCIONES[i]][0]
                y = self.direcciones[DIRECCIONES[i]][1]
                utilidad = self.mundo_aux[x][y]

                self.mundo_aux[self.coordenadas[0]][self.coordenadas[1]] = 0
                nuevo_mundo_aux = self.mundo_aux.copy() 
                nuevo_mundo_aux[x][y] = self.tipo_jugador
    ...
    def verifica_movimiento(self, nueva_coordenada) -> bool:
        if (nueva_coordenada[0] >= 0 and nueva_coordenada[0] < len(self.mundo[0])
            ...
            and (self.mundo[nueva_coordenada[0]][nueva_coordenada[1]] != 8 and self.mundo[nueva_coordenada[0]][nueva_coordenada[1]] != 9)):
            return True
        else: return False
